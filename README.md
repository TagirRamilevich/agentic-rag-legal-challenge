...
A fully deterministic pipeline using BM25 retrieval + regex extraction. No external LLM API required.
...
# 3. Download corpus (warmup or final)
python scripts/download_corpus.py --phase warmup

# 4. Build submission (ingest → index → retrieve → answer → validate → archive)
...
Caches are stored in `.cache/<phase>/` so reruns skip ingestion and indexing.
